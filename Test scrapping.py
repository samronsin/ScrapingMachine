...
def nb_backers(project_page):
    return int(re.search(u'(\"Project\[backers_count\]\">)([0-9]+)' , webpage).groups()[1])
...
def project_goal(project_page):
    return float(re.search(u'(data-goal=\")([0-9]+\.[0-9]+)', webpage).groups()[1])

def nb_rewards(project_page):
    return int(re.search(u'(data-reward-count=\")([0-9]+)',webpage).groups()[1])
