...
def create_plot(crossvalid_share):
    X_train, X_test, y_train, y_test = cross_validation.train_test_split(statuses, projects[:,2], test_size=crossvalid_share, random_state=0)
    accuracies_test = []
    accuracies_test_l_ = []
    for date in dates:
    ...
    dic_plot1= {"markeredgecolor":str(crossvalid_share) , "linestyle":"-"}
    ax.plot(dates,accuracies_test, label = 'benchmark (TEST)'+str(crossvalid_share),  **dic_plot1)
    dic_plot2= {"markeredgecolor":str(crossvalid_share) , "linestyle":":"}
    ax.plot(dates,accuracies_test_l_, label = 'double logistic (TEST)'+str(crossvalid_share), **dic_plot2)
